...
CREATE POLICY "Users can view own invoices" ON invoices
    FOR SELECT USING (owner::uuid = auth.uid());

CREATE POLICY "Users can insert own invoices" ON invoices
    FOR INSERT WITH CHECK (owner::uuid = auth.uid());

CREATE POLICY "Users can update own invoices" ON invoices
    FOR UPDATE USING (owner::uuid = auth.uid());

CREATE POLICY "Users can delete own invoices" ON invoices
    FOR DELETE USING (owner::uuid = auth.uid());
...
CREATE POLICY "Users can view own invoice items" ON invoice_items
    FOR SELECT USING (
        EXISTS (
            ...
            AND invoices.owner::uuid = auth.uid()
        )
    );
